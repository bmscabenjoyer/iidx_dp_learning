"""
Window-by-window nearest-neighbour search using the v10 encoder.
...
CKPT      = Path('checkpoints/pretrain_half_v10/encoder_best.pt')
...
def embed_all(encoder, manifest, query_title, query_diftype):
    ...
    for _, row in tqdm(lv12.iterrows(), total=len(lv12), desc='embedding pool'):
        ...
        p1_wins, _, win_nc = window_chart_time_half_v10(enc, nc)
        T = p1_wins.shape[0]
        if T == 0:
            continue
...
def main():
    ...
    print(f'  {args.query.upper()} {args.diftype}  ec={qm0["ec"]:.1f}  hc={qm0["hc"]:.1f}'
          f'  — densest windows, nearest neighbours (v10 encoder)')
    print('═'*105)
