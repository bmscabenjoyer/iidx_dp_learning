"""
Frozen v8 half-chart encoder probe: time-based windows (8s, 150 BPM ref)
embed_dim=384, depth=4, uniform masking.
P1+P2 embeddings fused as [mean, diff] → PCA(128) + 8 note-profile → RidgeCV
5-fold CV vs v7 baseline.
"""
...
CKPT      = Path('checkpoints/pretrain_half_v9/encoder_best.pt')
...
def load_encoder():
    model = MAEModel(
        ...
        encoder_dim=EMBED_DIM, decoder_dim=128, num_heads=6, encoder_depth=4, use_rope=True,
...
def precompute(encoder, manifest):
    ...
    for _, row in tqdm(lv12.iterrows(), total=len(lv12), desc='embedding charts'):
        ...
        def embed_hand(wins):
            ...
            return encoder.embed_segment(x, bpm=bpm).mean(0).cpu().numpy()  # (384,)

        p1_emb = embed_hand(p1_wins)
        p2_emb = embed_hand(p2_wins)
        fused  = np.concatenate([(p1_emb + p2_emb) / 2.0, p1_emb - p2_emb])  # (768,)
...
print('\n── Results ─────────────────────────────────────────────────────────────', flush=True)
V8_RHO = {'rating_ec': 0.7400, 'rating_hc': 0.7243, 'rating_exh': 0.7017}
V8_MAE = {'rating_ec': 1.4863, 'rating_hc': 1.1202, 'rating_exh': 0.8456}
print(f'{"target":<12}  {"naive":>7}  {"XGB ρ":>7}  {"v8 ρ":>7}  {"v9 ρ":>7}  '
      f'{"XGB MAE":>8}  {"v8 MAE":>8}  {"v9 MAE":>8}')
print('─' * 90)
for j, t in enumerate(TARGETS):
    ...
    print(f'{t:<12}  {naive:7.4f}  {XGB_RHO[t]:7.4f}  {V8_RHO[t]:7.4f}  {rho:7.4f}  '
          f'{XGB_MAE[t]:8.4f}  {V8_MAE[t]:8.4f}  {mae:6.4f}±{mae_s:.4f}', flush=True)
