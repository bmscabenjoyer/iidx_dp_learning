"""
Modal training for IIDX DP causal event-token pretraining.

Architecture
------------
Per-timestep event tokens → CausalEventEncoder (256-dim, 4 layers, 8 heads)
→ next-token prediction on lane_bits / delta_bin / note_type
→ last-token hidden state as window embedding
...
Run:
    modal run modal_causal_pretrain.py

Download checkpoint:
    modal volume get iidx-ckpts causal_pretrain/encoder_best.pt \\
        checkpoints/causal_pretrain/encoder_best.pt
"""
...
image = (
    ...
    .add_local_file("causal_event_dataset.py","/app/causal_event_dataset.py")
    .add_local_file("causal_event_encoder.py","/app/causal_event_encoder.py")
)
...
app = modal.App("iidx-causal-pretrain")
...
def train(
    ...
    ckpt_name:       str   = "causal_pretrain_v2",
):
    import sys
    ...
    from causal_event_dataset import CausalEventDataset
    from causal_event_encoder import CausalEventModel, model_info

    device = torch.device("cuda")

    # ── Build event token caches (skips already-cached) ───────────────────────

    cache_dirs = []
    for level in levels:
        chart_dir = f"{DATA_DIR}/dp{level}_active/charts"
        cache_dir = f"{DATA_DIR}/event_cache/dp{level}"
        print(f"Tokenising dp{level} charts → {cache_dir}", flush=True)
    ...
    for cache_dir in cache_dirs:
        datasets.append(CausalEventDataset(
            cache_dir     = cache_dir,
    ...
    print(f"Total windows: {len(dataset)}", flush=True)
